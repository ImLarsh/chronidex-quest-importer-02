...
export function PokedexHeader({
  ...
  return <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 creative-shadow">
      <div className="container mx-auto px-4 py-6">
        {/* Top Bar - Logo and Actions with personality */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4 group">
            <div className="relative">
              <img src={myPokedexLogo} alt="MyPokédex Logo" className="h-16 w-auto transform group-hover:rotate-3 organic-hover transition-transform duration-300" />
            </div>
        ...
        <div className="relative mb-8">
          <div className="relative group">
            ...
            <Input placeholder="Search for your favorite Pokémon... 🔍" value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="pl-12 pr-6 h-14 text-base organic-border bg-card/50 backdrop-blur-sm border-2 border-dashed border-border/60 hover:border-primary/30 focus:border-primary organic-hover playful-shadow" />
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <Button variant={showFavorites ? "default" : "outline"} onClick={onToggleFavorites} className="h-11 px-6 organic-border organic-hover group relative overflow-hidden">
              <div className="relative z-10 flex items-center">
                <Heart className={cn("h-4 w-4 mr-2 transform group-hover:scale-110 organic-hover", showFavorites && "fill-current animate-pulse")} />
                My Favorites
                {favoritesCount > 0 && <Badge variant="secondary" className="ml-2 h-6 px-2 text-xs organic-border bg-accent animate-bounce">
                    {favoritesCount}
                  </Badge>}
              </div>
              {showFavorites && <div className="absolute inset-0 bg-gradient-to-r from-red-500/20 to-pink-500/20 animate-pulse"></div>}
            </Button>

            <Button variant="outline" onClick={onRandomPokemon} className="h-11 px-6 organic-border organic-hover group bg-gradient-to-r from-accent/10 to-secondary/10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="group">
              ...
              <Select value={selectedType} onValueChange={onTypeChange}>
                ...
                <SelectContent className="organic-border creative-shadow">
                  {pokemonTypes.map(type => <SelectItem key={type} value={type} className="capitalize organic-hover">
            ...
            <div className="group">
              ...
              <Select value={selectedGeneration} onValueChange={onGenerationChange}>
                ...
                <SelectContent className="organic-border creative-shadow">
                  {generations.map(gen => <SelectItem key={gen.value} value={gen.value} className="organic-hover">
            ...
            <div className="group">
              ...
              <Select value={selectedGame} onValueChange={onGameChange}>
                ...
                <SelectContent className="organic-border creative-shadow">
                  {games.map(game => <SelectItem key={game.value} value={game.value} className="organic-hover">
